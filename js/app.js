...
function _renderMessages(docs, user) {
  ...
  groups.forEach(group => {
    ...
    const avatarEl = '<div style="display:flex;flex-direction:column;align-items:center;align-self:flex-end;flex-shrink:0;width:42px;gap:2px">'
      + avatarImg
      + '<span style="font-size:10px;font-weight:600;color:' + (isAdminMsg ? 'var(--accent)' : 'var(--text2)') + ';text-align:center;width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="' + _escHtml(d0.senderName || '') + '">' + _escHtml(d0.senderName || '') + '</span>'
      + roleBadge
      + '</div>';
